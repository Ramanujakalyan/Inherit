...
angular.module('ngS3upload.config', []).
  ...
  service('S3Uploader', ['$http', '$q', '$window', function ($http, $q, $window) {
    ...
    this.upload = function (scope, uri, key, acl, type, accessKey, policy, signature, file) {
	  console.log(uri + ' ' + key + ' ' + acl + ' ' + type + ' ' + accessKey + ' '+ policy + ' ' + file);
...
angular.module('ngS3upload.directives', []).
  directive('s3Upload', ['$parse', 'S3Uploader', 'ngS3Config', '$ionicLoading', function ($parse, S3Uploader, ngS3Config, $ionicLoading) {
    return {
      restrict: 'AC',
      require: '?ngModel',
      replace: true,
...
angular.module('ngS3upload').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('theme/bootstrap2.html',
    ...
    "  <button class=\"btn btn-primary\" style=\"width:120px; height: 120px\" type=\"button\"><span ng-if=\"!filename\">Choose file</span><span ng-if=\"filename\">Replace file</span></button>\n" +
